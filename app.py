...
st.title("🏠 AURA - Smarter Floor Plan Generator")
st.markdown("Describe your building layout, and AURA will draw a more architecturally accurate floor plan with logical adjacency.")
...
def create_adjacency_layout(rooms):
    layout = []
    x, y = 20, 20
    row_height = 0
...
# --- DRAW FUNCTION ---
def draw_svg_plan(layout):
    dwg = svgwrite.Drawing(size=(1000, 800))
...
if st.button("Generate Floor Plan") and prompt:
    rooms = mock_parse(prompt)
    layout = create_adjacency_layout(rooms)
    svg_code = draw_svg_plan(layout)
